...
const game = (id, reds, blues, greens) => {
    return {id, reds, blues, greens}
};

function count(regex, array, id, i) {
    let color = data2a[(id - 1)][i].match(regex);
    if (color) {
...
data2a.forEach(day => {
    let id = data2a.indexOf(day) + 1;
    let reds = [];
    let blues = [];
    let greens = [];

    for (let i = 0; i < day.length; i++) {
        count(redRegex, reds, id, i);
        count(blueRegex, blues, id, i);
        count(greenRegex, greens, id, i);
    }

    const newGame = game(id, reds, blues, greens);
    games.push(newGame);
})
